...
def main():
    ...
    # ✅ Model setup
    model = MNISTDPModel(noise_factor=noise_factor, norm_c=NORM_C).to(device)
    optimizer = optim.Adam(model.parameters(), lr=LR)
    criterion = nn.CrossEntropyLoss()

    # ✅ Log file
    log_path = "outputs/mnist_model/epoch_eval_results.txt"
    os.makedirs("outputs/mnist_model", exist_ok=True)
    ...
    # ✅ Save model for MIA
    torch.save(model.state_dict(), "outputs/mnist_model/pytorch_model.bin")
    with open(log_path, "a") as f:
        f.write(f"\nTotal Training Time: {elapsed_minutes:.2f} minutes\n")

    print("✅ Training complete! Model saved to outputs/mnist_model/")
