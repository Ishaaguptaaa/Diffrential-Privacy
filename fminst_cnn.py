...
from dp_noise import get_noise_multiplier, _max_norm_clip

# ----------------- Model -----------------
class FMNISTDPModel(nn.Module):
...
def main():
    ...
    # ✅ Model setup
    model = FMNISTDPModel(noise_factor=noise_factor, norm_c=NORM_C).to(device)
    optimizer = optim.Adam(model.parameters(), lr=LR)
    criterion = nn.CrossEntropyLoss()

    # ✅ Log file
    log_path = "outputs_fmnist/fmnist_model/epoch_eval_results.txt"
    os.makedirs("outputs_fmnist/fmnist_model", exist_ok=True)
    ...
    # ✅ Save model for MIA
    torch.save(model.state_dict(), "outputs_fmnist/fmnist_model/pytorch_model.bin")
    with open(log_path, "a") as f:
        f.write(f"\nTotal Training Time: {elapsed_minutes:.2f} minutes\n")

    print("✅ Training complete! Model saved to outputs_fmnist/fmnist_model/")
